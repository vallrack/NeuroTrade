...
WORKER_PORT = int(sys.argv[1]) if len(sys.argv) > 1 else 5003
...
def check_inactivity():
    while True:
        ...
        if time.time() - last_activity > INACTIVITY_TIMEOUT:
            print(f"[ALPACA WORKER {WORKER_PORT}] Inactividad. Apagando...")
            os._exit(0)
...
def connect():
    ...
    session_active = True
    return jsonify({"status": "success", "message": "Conectado a Alpaca (Scaffold)"})
...
def disconnect():
    ...
    global session_active
    session_active = False
    return jsonify({"status": "success", "message": "Desconectado de Alpaca"})
...
def trade():
    update_activity()
    return jsonify({"status": "error", "message": "Trading no implementado en scaffold Alpaca"})
...
def ping():
    update_activity()
    return jsonify({"status": "ok", "worker": "alpaca", "port": WORKER_PORT})
