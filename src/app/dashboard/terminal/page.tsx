...
import { bridgeAnalyze, getBridgeModeLabel } from '@/lib/bridge';
...
export default function TerminalPage() {
  ...
  const botParamsRef2 = useRef(botParams);
  useEffect(() => {
    botParamsRef2.current = botParams;
  }, [botParams]);
  ...
  useEffect(() => {
    ...
    const fetchRealLogs = async () => {
      ...
      const now = Date.now();
      if (now - lastLogFetch < 10000) return;
      lastLogFetch = now;

      isFetching = true;
      try {
        // Usar el primer par activo desde Firestore, o EURUSD-OTC como fallback
        const activePair = botParamsRef2.current?.pairs?.[0] || 'EURUSD-OTC';
        ...
        if (data.success) {
          setLogs(prev => {
            const newLog = {
              ...
              timestamp: new Date(),
              message: `Telemetría [${data.pair}] RSI:${data.rsi?.toFixed(1)} ${data.direction} — Balance: $${data.balance?.toLocaleString()} — ${getBridgeModeLabel()}`,
              level: 'success',
            };
    ...
    <>
      ...
      <main className="p-4 md:p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto w-full h-[calc(100vh-64px)] flex flex-col">
          ...
          <Card className="bg-card/50 border-white/5 backdrop-blur-xl">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-mono font-bold">24ms</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-card/50 border-white/5 backdrop-blur-xl">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-mono font-bold">ACTIVO</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-card/50 border-white/5 backdrop-blur-xl">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-mono font-bold">AES-256</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-card/50 border-white/5 backdrop-blur-xl">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-mono font-bold">14.8%</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="flex-1 bg-zinc-950 border-white/10 overflow-hidden flex flex-col shadow-2xl relative">
          ...
          <CardContent
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-6 font-mono text-xs md:text-sm custom-scrollbar bg-black/40"
          >
            <div className="space-y-1.5">
              {logs?.map((log: any, idx: number) => {
                const dateObj = log.timestamp instanceof Date
                  ? log.timestamp
              ...
              {logs.length === 0 && (
                <div className="text-muted-foreground">
                  Recepción de telemetría iniciada. Esperando datos del mercado de IQ Option...
                </div>
              )}
        ...
        <div className="pt-2 flex flex-col sm:flex-row justify-between items-center gap-4 pb-4">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <Cpu className="h-3 w-3 text-muted-foreground" />
              <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Kernel: v7.0.2-Stable</span>
            </div>
            <div className="flex items-center gap-2">
