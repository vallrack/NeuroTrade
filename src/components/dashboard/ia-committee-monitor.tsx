...
import { Brain, ArrowUpCircle, ArrowDownCircle, Activity, Zap, ShieldCheck, Loader2, Info, Cpu, TrendingUp, BarChart3 } from 'lucide-react';
...
export function IACommitteeMonitor() {
  ...
  const fetchConsensus = async () => {
    // Si estamos ejecutando un trade, pausamos el monitor brevemente
  ...
  const handleAutoTrade = async (direction: 'CALL' | 'PUT') => {
    ...
    try {
      ...
      // Cooldown de 10 segundos para evitar ráfagas accidentales
      setTimeout(() => {
  ...
  useEffect(() => {
    fetchConsensus();
    // Actualización rápida cada 5 segundos para lectura "Real"
    ...
    return (
      <Card className="h-full bg-card/50 border-white/5 min-h-[400px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Sincronizando lectura real...</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card className="h-full bg-card/50 border-white/5 backdrop-blur-xl relative overflow-hidden flex flex-col shadow-2xl transition-all duration-500">
      <CardHeader className="pb-4 border-b border-white/5 bg-white/5">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
             <p className="text-[9px] font-bold text-muted-foreground uppercase">Precio IA</p>
             <p className="text-sm font-code text-primary font-bold">{data?.livePrice}</p>
          </div>
        </div>
      </CardHeader>
      ...
      {isExecuting && (
        <div className="absolute inset-0 bg-background/90 backdrop-blur-xl flex flex-col items-center justify-center z-50 animate-in fade-in duration-300">
           ...
           <p className="text-2xl font-headline font-bold text-primary animate-pulse">EJECUTANDO V7</p>
           <p className="text-[10px] text-muted-foreground mt-2 uppercase tracking-widest font-bold">Sincronizando túnel con IQ Option...</p>
        </div>
      )}
    </Card>
  );
