...
- Ejecuta en tu computadora: `python bridge_server.py` (o `start-bridge.bat`)
- Si usas la app en Vercel, expón el puerto con túnel:
  ```bash
  npx localtunnel --port 5000
  ```
- En la app: **Broker Link → MI PC** → pega la URL del túnel
- Si todo corre en local (`npm run dev` + `python bridge_server.py`): usa `http://localhost:5000`
